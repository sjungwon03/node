...
// Each entry is [id, schemeOnly, option]. A null option means that the
// builtin is not gated by a runtime option. Do not include internal modules.
// Keep option values out of the snapshot; they are read during pre-execution.
const builtinModulePolicies = [
  ['bench', true, null],
  ['bench/reporters', true, null],
...
const experimentalModuleOptions = new SafeMap();
for (let i = 0; i < builtinModulePolicies.length; i++) {
  ...
  if (option !== null) {
    experimentalModuleOptions.set(id, option);
  }
}
...
const publicBuiltinIds = builtinIds
  .filter((id) =>
    !StringPrototypeStartsWith(id, 'internal/') &&
      !experimentalModuleOptions.has(id),
  );
...
class BuiltinModule {
  ...
  static allowExperimentalModules(getOptionValue) {
    for (const { 0: id, 1: option } of experimentalModuleOptions) {
  ...
  // Internal tests must not be able to mutate the registration policy.
