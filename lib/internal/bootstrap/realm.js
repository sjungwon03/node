...
// Shared with the native code-cache test categories in src/node_builtins.cc.
const builtinModulePolicies = getBuiltinModulePolicies();
const schemelessBlockList = new SafeSet();
const experimentalModuleList = new SafeSet();
for (let i = 0; i < builtinModulePolicies.length; i++) {
  const { id, schemeOnly, option } = builtinModulePolicies[i];
  if (schemeOnly) {
    schemelessBlockList.add(id);
  }
  if (option !== null) {
    experimentalModuleList.add(id);
  }
}
...
class BuiltinModule {
  ...
  static allowExperimentalModules(getOptionValue) {
    for (let i = 0; i < builtinModulePolicies.length; i++) {
      const { id, option } = builtinModulePolicies[i];
      if (option !== null && getOptionValue(option)) {
